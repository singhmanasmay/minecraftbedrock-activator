...
import threading
import functools
import pyuac  # For handling admin privileges
...
def activate():
    ...
    # Check for admin privileges and system directories
    ...
    output('Activating Minecraft Bedrock Edition...\nPlease wait...','#0096FF')
    if init:
        init = False
        log('\n'.join(['██████╗░░█████╗░██╗░░░░░███████╗██████╗░░░███╗░░░█████╗░███╗░░██╗',
                       '██╔══██╗██╔══██╗██║░░░░░██╔════╝██╔══██╗░████║░░██╔══██╗████╗░██║',
    ...
    if admin:
        ...
        output('Administrator privileges not granted.\nPlease run this script as an administrator.','#FF0000')

# Entry point: Check for admin privileges and launch GUI
if not pyuac.isUserAdmin():
    try:
        pyuac.runAsAdmin()  # Try to elevate privileges
    except:
        gui()  # Fall back to GUI if elevation fails
else:
    gui()  # Launch GUI directly if already running as admin
